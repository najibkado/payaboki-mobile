...
class EscrowWallet extends StatefulWidget {
  final user_data;
  const EscrowWallet({Key? key, this.user_data}) : super(key: key);

  @override
  State<EscrowWallet> createState() => _EscrowWalletState();
}

class _EscrowWalletState extends State<EscrowWallet> {
  late String _email;
  late double _amount;
  bool verified = false;
  String _name = "";
  ...
  void _sendRequest(sender, recv, amount) {
    ...
    Services.escrowTransaction(body)
        .then((value) => {
              if (value)
                {
